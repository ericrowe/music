#!/usr/bin/env python3
"""Generate the refined 2026 Continuum Field Prop Operations Guide in Typst."""
...
#job-card(
  ...
  #text(size: 8.5pt, fill: rgb("#4a5568"))[
    *Day-of Directive Example:* The Prop Lead will assign your exact coordinates based on the venue's exit gate: \
    _“You are on Side 1, stage at Back 20, and roll towards the center (41-yd line)”_ or \
    _“You are on Side 2, stage at Back 40, and roll towards the end zone (80-yd line).”_
  ]
  #v(2pt)

  + *Pre-Show Staging:* Report to Equipment Trailer 45 minutes before step-off. Push your loaded cart (8 screens + 16 double-bagged sandbags) to the rear stadium entrance gate. Queue on the back sideline at your assigned yard line marker (e.g., Back 20 on the gate side, Back 40 on the far side to eliminate traffic bottlenecking).
  + *Field Ingress:* When the CBA judge signals entry permission, push the cart forward around the perimeter track toward the front sideline. Coordinate pace with your Student Unloader walking alongside.
  + *Directional Delivery (Rolling Away from Exit Gate):* Enter the front sideline at your starting yard mark and roll *away* from the exit gate (e.g., from the 20 toward the 41, or from the 59 toward the 80). Pause for ~6–8 seconds at each 2-yard mark while the unloader deposits one folded screen and two sandbags to the pre-set student receiver.
  + *Cart Clearance Before Announcement:* Once your 8th screen is delivered, immediately push the empty cart across the front boundary into the front staging area. *You must be off the turf before the 2:15 mark.* Park the cart facing the stadium exit gate ready for post-show egress.
  + *Post-Show Zero-Doubling-Back Ballast Sweep:* On the final show chord, re-enter the front sideline starting at the far screen and roll *toward the stadium exit gate*. Scoop ballast bags placed on the turf by students into the cart hopper as you advance without doubling back. Keep moving continuously across the exit gate threshold to stop the official CBA 2:00 egress clock. Do not stop until you reach the trailer lot!
]
...
output_path = Path("PCHSMB/2026 Continuum/Continuum_2026_Prop_Operations_Guide.typ")
