...
> High school competitive marching shows typically run **7:45 to 8:30**. Designing a show up to **9:45 of continuous performance** guarantees a **100.0% zero-penalty probability**, leaving over 45 seconds of pure contingency buffer before the 99.999% 5-sigma extreme rail.
...
* **Dynamic Tradeoff Mechanics:**
  - With the Two-Student Carry deploying all 16 duck blinds in **55.7 seconds** (saving **139.3 seconds** compared to the 3:15 cap), the director signals early.
  - The announcement (~35s) and performance (~8:30) start and finish earlier on the master clock.
  - **That saved 139 seconds transfers directly into the egress and performance budget**, expanding the post-show buffer and eliminating all time pressure!
...
* **Standard Stadiums & The Exit Gate Bottleneck:**
  ...
  - Narrow double-swing or service gates frequently create a physical bottleneck where front ensemble carts, large backfield props, duck blinds, and marching students converge into a single exit lane. Two-Student Carry minimizes congestion because student pairs jog directly through the gate without stopping to load carts.
* **Falcon Stadium Tunnel Protocol (State Championships - USAFA):**
  ...
  - Inside the tunnel mouth, student pairs and props are completely off the competition clock while the next band enters and sets up in their 3:15 window.
...
```mermaid
flowchart LR
    TSC1["<b>Two-Student Carry</b><br/>16 Pairs (32 Students)<br/><b>13 lbs / Student</b>"]
    TSC2["<b>Parallel Walk-Across</b><br/>53.3-yd straight transit<br/>0 turns, 0 traffic"]
    TSC3["<b>Rapid Set & Ballast</b><br/>Drop on mark (~6s)<br/>Place ballast (~4s)"]
    TSC4["<b>Time: 55.7s (0:56)</b><br/>★ 100.0% Success ★<br/>+139.3s Banked Slack"]
    TSC1 --> TSC2 --> TSC3 --> TSC4
...
![Pre-Show Deployment Probability Distribution (Two-Student Carry vs Carts)](../../simulation/plots/cdf_comparison.png)
...
| Fleet Config | Starting Location | Setup Strategy | Mean Time | Median | P95 Time | P99 Time | Success Rate ($T \le 3:15$) | Safety Slack | Tactical Recommendation |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---|
...
![Pusher & Ballast Sensitivity Analysis](../../simulation/plots/sensitivity_analysis.png)
...
## 4. Post-Show Egress Simulation Analysis ($N = 5,000$ Trials)
...
sequenceDiagram
    autonumber
    actor Field as 16 Student Pairs on Sideline
    actor Ballast as 2 Adult Handlers (Sideline)
    participant Line as Exit Boundary Line / Chute
    participant Tunnel as Falcon Stadium Tunnel (Off-Clock)

    Note over Field,Tunnel: SHOW CONCLUDES (T = 0:00)
    Field->>Field: Lift ballast bags gently to turf (2-4s)
    Field->>Line: Jog down sideline carrying assembled screens (13 lbs/student) (30-50s)
    Field->>Tunnel: Cross exit boundary line into tunnel mouth (T ≈ 45 - 55s)
    Ballast->>Ballast: Sweep resting ballast bags off sideline (15-45s)
    Ballast->>Line: Push ballast carts / carriers across exit boundary line (T ≈ 50 - 55s)
    Note over Line,Tunnel: ★ MASTER CBA 15:00 TIMING CLOCK STOPS (T ≈ 55s) ★
    Note over Tunnel: CONTEST CLOCK STOPPED — ALL CLEAR
    Field->>Tunnel: Walk directly through exit chute to equipment trailer lot (No reloading!)
```
...
* **Simulation Baseline:** High-Precision Stochastic Engine ($N = 5,000$ Randomized Trials per Scenario)
* **Authoritative Cross-References:**
  - Technical Specification: [`TECHNICAL_SPEC.md`](TECHNICAL_SPEC.md)
  - Semicircular Wind Relief Cuts Specification: [`WIND_RELIEF_CUTS_SPECIFICATION.md`](WIND_RELIEF_CUTS_SPECIFICATION.md)
