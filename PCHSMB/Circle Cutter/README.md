...
3. **Full-Width Unibody End Cap & Clamp (Piece 3):** A protective unibody end cap ($17.72\text{ mm}\text{ L} \times 64.0\text{ mm}\text{ W} \times 28.0\text{ mm}\text{ H}$) spanning the entire distal chevron face of the arm. Features a **100% planar flat front surface** ($X = 115.50\text{ mm}$) for rock-solid, face-down build plate adhesion with **zero supports**, ensuring all cylindrical bores and counterbores print in the vertical $Z_{\text{printer}}$ axis for maximum hole roundness and tight tolerance. Clamped to the arm solely via the single uxcell $\varnothing 4.0\text{ mm} \times 10.0\text{ mm}$ M3 shoulder bolt through the 28mm blade bore, maintaining **$0.45\text{ mm}$ of axial running float** for silky-smooth, zero-binding rotation on the ground steel shoulder without wobble or flex. The shoulder screw post, through-bore ($\varnothing 4.65\text{ mm}$ with $+0.65\text{ mm}$ print clearance for effortless bolt insertion), and counterbore ($\varnothing 8.2\text{ mm}$ with $+1.2\text{ mm}$ clearance for socket head and hex driver) are strictly aligned along the true canted blade axle vector at **$+9.0607^\circ$**, matching the arm's standoff boss with zero kinking. The bearing side has **zero screw holes**, featuring a true circular arched overhead canopy ($12.5\text{ mm}$ depth, true circular radius $R = 13.0\text{ mm}$, concentric with bearing axle at $Z_{\text{local}} = 7.0\text{ mm}$) that completely covers the 608 roller bearing from above while remaining open at the bottom so the bearing rolls freely directly on the vinyl ($Z_{\text{world}} = 0.0\text{ mm}$). The blade side features a true circular relief cavity ($R = 15.0\text{ mm}$, concentric with blade axle at $Z_{\text{local}} = 9.5\text{ mm}$) backed by an $8.0\text{ mm}$ solid PETG core. The end cap bottom is suspended $4.0\text{ mm}$ in the air above the vinyl ($Z_{\text{world}} = +4.0\text{ mm}$), preventing any plastic drag. The rear mating face matches the arm's $18.12^\circ$ included chevron angle, providing positive kinematic anti-rotation without secondary screws.
...
| **Piece 3: Full-Width Unibody End Cap**<br>[`circle_cutter_blade_cap.stl`](circle_cutter_blade_cap.stl) | $17.72 \times 64.0 \times 28.0\text{ mm}$ | PETG / 100% Solid (~40.2g) | • Full-width unibody end cap ($64.0\text{ mm}\text{ W} \times 28.0\text{ mm}\text{ H} \times 17.72\text{ mm}\text{ D}$) spanning entire distal chevron face of the arm<br>• **100% Flat Front Print Face ($X = 115.50\text{ mm}$):** Enables flat face-down build plate adhesion with zero supports; all bores and counterbores print vertically for maximum circularity and tight tolerances<br>• **True Canted Axle Centerline (+9.0607°):** Shoulder screw post, bore ($\varnothing 4.65\text{ mm}$), and counterbore ($\varnothing 8.2\text{ mm}$) strictly aligned with canted blade vector $\vec{n}_b = (\cos\theta_c, \sin\theta_c, 0.0)$, matching arm blade boss with zero kink<br>• Single-screw retention: clamped solely via single uxcell $\varnothing 4.0\text{ mm} \times 10.0\text{ mm}$ M3 shoulder screw through 28mm blade bore into arm's M3 heat-set insert<br>• 28mm blade enclosure: true circular cylinder cavity ($R = 15.0\text{ mm}$, $R_y = R_z = 15.0\text{ mm}$) concentric with blade axle at $Z_{\text{local}} = 9.5\text{ mm}$ with central clamping hub ($\varnothing 9.0\text{ mm}$) providing **$0.45\text{ mm}$ axial running float** for silky-smooth blade rotation; backed by solid $8.0\text{ mm}$ core<br>• 608 bearing canopy: generous true circular arched overhead canopy ($12.5\text{ mm}$ depth, $R = 13.0\text{ mm}$, $R_y = R_z = 13.0\text{ mm}$) strictly concentric with bearing axle at $Z_{\text{local}} = 7.0\text{ mm}$ with **zero screws on bearing side**; open bottom allows bearing to roll directly on vinyl at $Z_{\text{world}} = 0.0\text{ mm}$<br>• Bottom of cap elevated at $Z_{\text{local}} = 0.0\text{ mm}$ ($Z_{\text{world}} = +4.0\text{ mm}$), guaranteeing **$4.0\text{ mm}$ air clearance above vinyl** so plastic never rubs or drags on vinyl<br>• $\varnothing 8.2\text{ mm}$ counterbore along canted axle ray with socket head recessed $0.24–1.48\text{ mm}$ below flat front face |
...
* **Print Orientations:**
  ...
  * **Piece 3 (Full-Width Unibody End Cap):** **Print face-down on the flat front surface ($X = 115.50\text{ mm}$).** 100% flat bed adhesion with zero supports; all cylindrical holes and counterbores print in the vertical $Z_{\text{printer}}$ axis for maximum circularity and tight tolerances (~45 min).
...
* **v2.1 — Full-Width Unibody End Cap with Bearing Canopy & Single M3 Fastener Attachment ([Plan 009](Plans/009-full-width-end-cap.md)):**
  ...
  - Redesigned Piece 3 (`circle_cutter_blade_cap.stl`) as a Full-Width Unibody End Cap spanning the full distal chevron face.
  ...
  - Bearing clearance canopy: Bearing side has 0 screw holes; an arched overhead canopy ($12.5\text{ mm}$ depth, $R = 13.0\text{ mm}$) covers the 608 roller bearing from above with an open bottom allowing the bearing to roll freely on vinyl at $Z_{\text{world}} = 0.0\text{ mm}$.
  - Eliminated oval cavity distortion: Established true circular cylinder cuts ($R_y = R_z = R$) concentric with blade axle ($R = 15.0\text{ mm}$) and bearing axle ($R = 13.0\text{ mm}$).
* **v2.2 — Flat Front Print Surface & True Canted Axle Centerline for Piece 3 End Cap:**
  - **100% Flat Front Build-Plate Surface:** Established a single uniform plane at $X = 115.50\text{ mm}$ across the entire front face ($Y \in [-32.0, +32.0\text{ mm}], Z \in [0.0, 28.0\text{ mm}]$), replacing the prior V-shaped chevron front. Enables placing the part 100% flat face-down on the print bed with zero rocking and zero build-plate supports.
  - **Vertical Cylindrical Hole Precision:** By printing face-down on the flat front, all cylindrical features (screw counterbore, through-bore, hub post, and cavities) print as vertical circles in the printer's XY slicing plane, eliminating hole sagging/distortion and achieving perfect dimensional tolerances.
  - **True Canted Axle Centerline (+9.0607°):** Fully aligned the shoulder screw through-bore, head counterbore, and internal clamping hub post ($\varnothing 9.0\text{ mm}$) with the arm's true canted blade normal vector $\vec{n}_b = (\cos\theta_c, \sin\theta_c, 0.0)$, strictly coaxial from the rear mating face to the front face and eliminating the prior straight-in ($X$-axis parallel) kink.
  - **Recessed Screw Head & Running Float:** Screw counterbore seat is positioned at $s = 11.50\text{ mm}$ along the canted axle ray ($1.5\text{ mm}$ arm standoff boss + $10.0\text{ mm}$ shoulder bolt length). The $3.0\text{ mm}$ socket cap head is completely recessed below the flat front face ($0.24–1.48\text{ mm}$ sub-flush). Hub post tip at $s = 2.30\text{ mm}$ maintains exact $9.20\text{ mm}$ bore length, providing $0.80\text{ mm}$ exposed steel shoulder and **$0.45\text{ mm}$ axial running float** for the $0.35\text{ mm}$ Fiskars blade.
  - **Cavity Floor & Solid Core:** Cavity floor set at $s = 3.50\text{ mm}$ along the canted axle ray ($1.65\text{ mm}$ clear air gap forward of blade), backed by a rigid $8.0\text{ mm}$ solid PETG bulkhead to the counterbore seat and bounding the cavity within $Y \le 31.36\text{ mm}$ to maintain the exact $64.00\text{ mm}$ outer width.
* **v2.2.1 — Shoulder Bolt Clearance & Counterbore Tolerance Opening for Piece 3 End Cap:**
  - **Bore Tolerance Opened to $\varnothing 4.65\text{ mm}$:** Physical print testing revealed that the modeled $\varnothing 4.20\text{ mm}$ through-bore printed slightly undersized ($\approx 3.85 - 3.95\text{ mm}$) due to FDM hole perimeter shrinkage and layer stepping over the $9.20\text{ mm}$ bore depth at a $9.06^\circ$ cant, binding against the $\varnothing 4.00\text{ mm}$ steel shoulder bolt shaft. Increased `GUARD_BORE_DIA` to **$\varnothing 4.65\text{ mm}$** ($+0.65\text{ mm}$ CAD clearance; $\approx 4.35\text{ mm}$ printed ID), ensuring an effortless, smooth slip fit with zero binding and zero need for post-print drilling.
  - **Counterbore Tolerance Opened to $\varnothing 8.20\text{ mm}$:** Increased `GUARD_CB_DIA` from $7.80\text{ mm}$ to **$8.20\text{ mm}$** ($+1.20\text{ mm}$ CAD clearance; $\approx 7.90\text{ mm}$ printed ID), allowing standard hex wrenches and bit drivers to seat and drive the 7.0mm socket head freely without wall interference.
  - **Preserved Clamping & Float:** Wide $1.175\text{ mm}$ radial shelf ($21.50\text{ mm}^2$ bearing area) under the bolt head, $2.175\text{ mm}$ solid hub wall thickness (5+ perimeters), exact $0.45\text{ mm}$ axial running float, and $4.0\text{ mm}$ vinyl ground clearance fully maintained.
...
*(No pending enhancements currently in queue. All v2.2 engineering requirements are fully implemented and verified.)*
